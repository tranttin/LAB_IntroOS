...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        // Kiểm tra xem start_row và end_row có hợp lệ không
        if (threadData[i].start_row >= result_rows) {
            fprintf(stderr, "Lỗi: start_row không hợp lệ cho thread %d.\n", i);
            exit(EXIT_FAILURE);
        }
        if (threadData[i].end_row > result_rows) {
            threadData[i].end_row = result_rows;
        }
